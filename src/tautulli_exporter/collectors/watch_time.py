...
class WatchTimeCollector(BaseCollector):
    ...
    async def collect(self) -> None:
        ...
        for lib in libs:
            ...
            try:
                media_table = await self.client.get_library_media_info(section_id=section_id)
            except Exception as e:
                logger.warning(f"Failed to fetch media info for library {library_name}: {e}")
                continue
            ...
            for item in items:
                if processed >= self.max_items:
                    logger.debug("WatchTimeCollector reached max_items cap")
                    return
                ...
                try:
                    stats = await self.client.get_item_watch_time_stats(rating_key=rating_key, media_type=media_type, query_days="0")
                    ...
                    if media_type == "episode":
                        show_key = str(item.get("grandparent_rating_key") or item.get("grandparent_key") or "")
                        show_title = item.get("grandparent_title") or item.get("grandparent_full_title") or item.get("grandparent") or "unknown"
                    ...
                    show_id = (show_key, show_title[:100], show_media_type, library_name)
                    show_totals[show_id] = show_totals.get(show_id, 0) + total_seconds
                    processed += 1

                except Exception as e:
                    logger.warning(f"Failed to get watch time for {title} ({rating_key}): {e}")

        # Publish aggregated show-level totals
        try:
            for (show_key, show_title, show_media_type, lib_name), seconds in show_totals.items():
